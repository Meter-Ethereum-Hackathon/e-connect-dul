...
cardrouter.post("/addPublicCard", async (req, res) => {
    const card = req.body;
    card.username = req.session.user;
    console.log("addPublicCard session user", req.session.user);
    console.log("card:", card);

    const ret = await myDB.addPublicCard(card.username, card);
    if (ret) {
        res.send("successful");
    } else {
        res.send("failure");
